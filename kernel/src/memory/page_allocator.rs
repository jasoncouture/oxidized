...
impl PageAllocator {
    ...
    pub(crate) fn initialize(hal: &mut Hal) {
        ...
        let max_address = memory_map.iter().max_by(|left, right| left.end.cmp(&right.end)).unwrap();
    ...
    pub(crate) fn force_allocate_pages(start_address: PlatformMemoryAddress, page_count: usize) -> Result<(), PageTrackerError> {
        let mut tracker = PAGE_TRACKER.lock();
    ...
    pub(crate) fn allocate_range(start_address: PlatformMemoryAddress, page_count: usize) -> Result<PlatformMemoryAddress, PageTrackerError> {
    ...
    pub(crate) fn free_range(start_address: PlatformMemoryAddress, page_count: usize) -> Result<(), PageTrackerError> {
        let mut tracker = PAGE_TRACKER.lock();
