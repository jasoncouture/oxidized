...
impl VgaTextWriter {
    ...
    pub fn write_byte(&mut self, byte: u8) {
        match byte {
            ...
            byte => {
                ...
                self.buffer.screen_buffer[row][col].write(ScreenChar {
    ...
    fn new_line(&mut self) {
        for row in 1..BUFFER_HEIGHT {
            for col in 0..BUFFER_WIDTH {
                let character = self.buffer.screen_buffer[row][col].read();
                self.buffer.screen_buffer[row - 1][col].write(character);
            }
        }
    ...
    fn clear_row(&mut self, row: usize) {
        ...
        for col in 0..BUFFER_WIDTH {
            self.buffer.screen_buffer[row][col].write(blank);
        }
    }
...
lazy_static! {
    pub static ref WRITER: Mutex<VgaTextWriter> = Mutex::new(VgaTextWriter {
        column_position: 0,
